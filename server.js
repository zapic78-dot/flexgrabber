...
let filters = {
  ...
  startTime: '07:00',
  endTime: '20:00',
  blockType: 'both', // 'both' | 'instant' | 'scheduled'
};

let flexToken = null;  // Set via /api/auth
...
// ─── Amazon Flex API calls ─────────────────────────────────────
// NOTE: Replace these with real Amazon Flex API endpoints.
// Amazon Flex uses https://flex-capacity-na.amazon.com for North America.
// You need a valid flexToken (Bearer token from the Flex app).

async function fetchOffersFromAmazon() {
  if (!flexToken) {
    // Return mock data when no real token is set (demo/dev mode)
  ...
  try {
    ...
    return raw.map(o => ({
      ...
      currency: o.rateInfo?.priceCurrencyCode || 'USD',
...
async function acceptOffer(offerId) {
  if (!flexToken) {
    // Mock accept in demo mode
    addLog(`[Demo] Accepted offer ${offerId}`, '#FF9900');
    return true;
  }
  try {
    await axios.post(
      `https://flex-capacity-na.amazon.com/v0/acceptOffer`,
      { offerId },
      {
        headers: {
          Authorization: `Bearer ${flexToken}`,
...
const STATION_NAMES = [
  { code: 'SEA5', name: 'SEA5 — Kent, WA' },
  { code: 'DWA3', name: 'DWA3 — Renton, WA' },
  { code: 'SEA8', name: 'SEA8 — Tukwila, WA' },
  { code: 'DWA5', name: 'DWA5 — Auburn, WA' },
...
function getMockOffers() {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const type = Math.random() > 0.4 ? 'instant' : 'scheduled';
    ...
    return {
      ...
      currency: 'USD',
      durationHours: hrs,
      startTime: `${startHr}:00`,
      endTime: `${startHr + hrs}:00`,
      type,
...
async function botTick() {
  ...
  if (matched.length > 0) {
    ...
    for (const offer of matched) {
      ...
      if (ok) {
        botState.grabbed++;
        botState.earned += offer.pay;
        addLog(`Grabbed: ${offer.station} — $${offer.pay} / ${offer.durationHours}h`, '#FF9900');
      }
    }
  } else {
  ...
  // Update live offer list with all offers (not just matched)
  botState.offers = offers;
}

// ─── Routes ───────────────────────────────────────────────────

// Auth: store the flex token
app.post('/api/auth', (req, res) => {
  const { token } = req.body;
  if (!token) return res.status(400).json({ error: 'Token required' });
  flexToken = token;
  addLog('Amazon Flex account connected', '#1D9E75');
...
app.post('/api/bot/start', (req, res) => {
  ...
  pollInterval = setInterval(botTick, 4000); // poll every 4 seconds
...
app.post('/api/filters', (req, res) => {
  ...
  if (stations !== undefined) filters.stations = Array.isArray(stations) ? stations : stations.split(',').map(s => s.trim());
  ...
  addLog('Preferences updated', '#1D9E75');
...
app.listen(PORT, () => console.log(`FlexGrabber backend running on port ${PORT}`));
